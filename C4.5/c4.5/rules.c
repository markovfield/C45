...
/*  ----------  */
{
    ...
    RStreamOut((char *) &NRules, sizeof(RuleNo));
    RStreamOut((char *) &DefaultClass, sizeof(ClassNo));
 
    ForEach(ri, 1, NRules)
    {
	r = RuleIndex[ri];
        RStreamOut((char *) &Rule[r].Size, sizeof(short));
	ForEach(d, 1, Rule[r].Size)
	{
	    Tst = Rule[r].Lhs[d]->CondTest;
 
	    RStreamOut((char *) &Tst->NodeType, sizeof(short));
	    RStreamOut((char *) &Tst->Tested, sizeof(Attribute));
	    RStreamOut((char *) &Tst->Forks, sizeof(short));
	    RStreamOut((char *) &Tst->Cut, sizeof(float));
	    if ( Tst->NodeType == BrSubset )
	    {
		Bytes = (MaxAttVal[Tst->Tested]>>3) + 1;
		ForEach(v, 1, Tst->Forks)
		{
		    RStreamOut((char *) Tst->Subset[v], Bytes);
		}
	    }
	    RStreamOut((char *) &Rule[r].Lhs[d]->TestValue, sizeof(short));
	}
	RStreamOut((char *) &Rule[r].Rhs, sizeof(ClassNo));
	RStreamOut((char *) &Rule[r].Error, sizeof(float));
    }
}
...
/*************************************************************************/
/*								  	 */
/*  Put n characters starting from address s to file Rf		  	 */
/*								  	 */
/*************************************************************************/
...
/*  ---------  */
{
    ...
    RStreamIn((char *) &nr, sizeof(RuleNo));
    RStreamIn((char *) &DefaultClass, sizeof(ClassNo));
 
    ForEach(r, 1, nr)
    {
        RStreamIn((char *) &n, sizeof(short));
	Cond = (Condition *) calloc(n+1, sizeof(Condition));
	ForEach(d, 1, n)
	{
	    Tst = (Test) malloc(sizeof(struct TestRec));
 
	    RStreamIn((char *) &Tst->NodeType, sizeof(short));
	    RStreamIn((char *) &Tst->Tested, sizeof(Attribute));
	    RStreamIn((char *) &Tst->Forks, sizeof(short));
	    RStreamIn((char *) &Tst->Cut, sizeof(float));
	    if ( Tst->NodeType == BrSubset )
	    {
		...
		ForEach(v, 1, Tst->Forks)
		{
		    ...
		    RStreamIn((char *) Tst->Subset[v], Bytes);
		}
	    }
 
	    Cond[d] = (Condition) malloc(sizeof(struct CondRec));
	    Cond[d]->CondTest = FindTest(Tst);
	    RStreamIn((char *) &Cond[d]->TestValue, sizeof(short));
	}
	RStreamIn((char *) &c, sizeof(ClassNo));
	RStreamIn((char *) &e, sizeof(float));
	NewRule(Cond, n, c, e);
	cfree(Cond);
    }
}
...
/*************************************************************************/
/*								  	 */
/*  Get n characters from file Rf into address s and onwards	  	 */
/*								  	 */
/*************************************************************************/
    ...
    Condition c;
{
	...
	case BrSubset:
	    printf(" in ");
	    ForEach(pv, 1, MaxAttVal[tp->Tested])
	    {
		if ( In(pv, tp->Subset[v]) )
		{
		    ...
		    printf("%s", AttValName[tp->Tested][pv]);
		}
	    }
	    printf("}\n");
    }
